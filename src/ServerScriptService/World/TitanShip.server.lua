...
-- TitanShipData.lua, an auto-generated table of ~3757 part records
...
--
-- Hull/entrance safety: unlike MegaWreckShip there is no separate breach
-- bookkeeping here, and none is needed -- every panel (Cloison, Coque,
-- Sol...) is placed at its own exact real position from the source file,
-- so wherever the original designer left a real gap (a doorway, a
-- porthole run, an open deck edge) that gap exists in Roblox too, simply
-- by including every record faithfully. There is no simplified/merged
-- collider standing in for the real skin this time -- see MegaWreckShip's
-- header comment for the "invisible wall" bug that caused, which this
-- data shape cannot reproduce (each record is already an individual
-- exact panel, never a stand-in for several at once).
