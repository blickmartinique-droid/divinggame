...
--
-- Built by WorldBootstrap after the ocean and caves and before the
-- currents: it publishes the ship's frame, its reserved bounding box and
-- the open-water spot off its stern (layout anchors) so the Épave vortex
-- and the seabed mound under the keel are placed from the real hull,
-- not from guessed coordinates.
...
function MegaWreckShip.Build(layout)
	-- Folders -----------------------------------------------------------------------
	...
	for roomName, count in pairs(LOOT_ROOM_COUNTS) do
		...
		if floorRecord then
			...
			-- Attributes before the tag: a spawner reacting to the tag must
			-- already see RegionKind.
	...
	do
		...
		exteriorRegion:SetAttribute("RegionSpecies", "RequinRecif,MeduseLumineuse") -- sharks patrol, jellyfish drift in the dark around the hull (mantas live shallower than 320 m)
	...
	local function scatterDebris(rng: Random)
		local halfLength = 100 -- raw studs; half the ~201-stud hull length
		for i = 1, 40 do
			local along = (rng:NextNumber() - 0.5) * halfLength * 2
			local across = (rng:NextNumber() - 0.5) * 40 -- raw studs; a bit wider than the ~36 beam
			local local_ = Vector3.new(along, -18 + rng:NextNumber() * 10, across) * SHIP_SCALE
			local worldPos = (shipCFrame * CFrame.new(local_)).Position
			local kind = rng:NextNumber()
			local size = 1.5 + rng:NextNumber() * 3
			local piece
			if kind < 0.4 then
			...
			piece.CFrame = CFrame.new(worldPos) * CFrame.Angles(rng:NextNumber() * math.pi, rng:NextNumber() * math.pi, rng:NextNumber() * math.pi)
			piece.Parent = debrisFolder
		end
	end
	scatterDebris(layout:Random("WreckDebris"))
	...
	-- Layout: the hull's real bounding box (every part's corners, in the
	-- ship's own frame), the stern's open water for the vortex, and keel
	-- samples for the seabed mound built under it.
	local minCorner = Vector3.new(math.huge, math.huge, math.huge)
	local maxCorner = -minCorner
	for _, record in ipairs(WreckData) do
		local frame = CFrame.fromMatrix(remapAxes(record.Center) * SHIP_SCALE, remapAxes(record.Right), remapAxes(record.Up))
		local half = partSize(record) / 2
		for _, sx in ipairs({ -1, 1 }) do
			for _, sy in ipairs({ -1, 1 }) do
				for _, sz in ipairs({ -1, 1 }) do
					local corner = frame * Vector3.new(half.X * sx, half.Y * sy, half.Z * sz)
					minCorner = minCorner:Min(corner)
					maxCorner = maxCorner:Max(corner)
				end
			end
		end
	end
	local localCenter = (minCorner + maxCorner) / 2
	layout:ReserveBox("MegaWreckShip", shipCFrame * CFrame.new(localCenter), maxCorner - minCorner)
	layout:SetAnchor("WreckCFrame", shipCFrame)
	layout:SetAnchor("WreckLocalMin", minCorner)
	layout:SetAnchor("WreckLocalMax", maxCorner)
	-- Open water just aft of the stern (raw local X ~-77 is the stern;
	-- -95 clears it), where the Épave vortex swirls around, not inside,
	-- the hull.
	layout:SetAnchor("WreckVortex", (shipCFrame * CFrame.new(Vector3.new(-95, 5, 20) * SHIP_SCALE)).Position)
	local keel = {}
	for x = minCorner.X, maxCorner.X, 30 do
		table.insert(keel, (shipCFrame * CFrame.new(x, minCorner.Y, (minCorner.Z + maxCorner.Z) / 2)).Position)
	end
	layout:SetAnchor("WreckKeel", keel)
	return shipFolder
end

return MegaWreckShip
