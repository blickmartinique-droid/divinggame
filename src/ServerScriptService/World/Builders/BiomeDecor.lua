-- Procedural set dressing so the ocean has shape and silhouettes at every
-- depth, built from cheap anchored primitives (no meshes, no textures).
-- Last WorldBootstrap step: it draws from a seeded Random (same map every
-- start) and asks the layout before placing anything big, so nothing here
-- grows through the wreck, a cave mountain or a current's path.
--
...
--   * Grottes arch: a landmark over the Grottes corridor current.
...
function BiomeDecor.Build(layout)
	...
	local function randomRotation(): CFrame
		return CFrame.Angles(random() * math.pi * 2, random() * math.pi * 2, random() * math.pi * 2)
	end
		...
		if y then
			...
			if kind < 0.45 then
				...
				for _ = 1, rng:NextInteger(3, 5) do
					...
					local tilt = CFrame.Angles((random() - 0.5) * 0.9, random() * math.pi * 2, (random() - 0.5) * 0.9)
			...
			elseif kind < 0.75 then
				...
				local width = 2 + random() * 2
				prop(reef, "CoralFan", Vector3.new(width, width * 0.9, 0.2), CFrame.new(base + Vector3.new(0, width * 0.45, 0)) * CFrame.Angles(0, random() * math.pi, 0), Enum.Material.Fabric, color)
			end
		end
	end
	...
	for _ = 1, 40 do
		...
		local y = shelfY(math.sqrt(x * x + z * z))
		if y then
			local height = 6 + random() * 8
			local lean = CFrame.Angles((random() - 0.5) * 0.25, random() * math.pi * 2, (random() - 0.5) * 0.25)
	...
	for _ = 1, 26 do
		...
		local size = 3 + random() * 9
		prop(reef, "Boulder", Vector3.new(size, size * (0.6 + random() * 0.5), size * (0.7 + random() * 0.5)), CFrame.new(x, y + size * 0.2, z) * randomRotation(), Enum.Material.Slate, Color3.fromRGB(78, 90, 104))
	end
	...
	for i = 1, PINNACLE_COUNT do
		local angle = (i / PINNACLE_COUNT) * math.pi * 2 + random() * 0.4
		local radius = 320 + random() * 360
		local x, z = math.cos(angle) * radius, math.sin(angle) * radius
		local topY = -60 - random() * 380
		local height = topY - FLOOR_Y
		local width = 22 + random() * 34
		-- Keep clear of the wreck, the mountains and every current's path.
		if not layout:IsSegmentFree(Vector3.new(x, FLOOR_Y, z), Vector3.new(x, topY + 10, z), width * 0.7 + 6) then
			continue
		end
		local color = Color3.fromRGB(60, 70, 84)
		prop(pinnacles, "Pinnacle", Vector3.new(width, height, width * (0.7 + random() * 0.6)), CFrame.new(x, FLOOR_Y + height / 2, z) * CFrame.Angles(0, random() * math.pi, 0), Enum.Material.Slate, color)
		-- A couple of ledges break the silhouette.
		for _ = 1, 2 do
			local ledgeY = FLOOR_Y + height * (0.3 + random() * 0.6)
			local ledge = width * (0.8 + random() * 0.8)
			prop(pinnacles, "Ledge", Vector3.new(ledge, 6 + random() * 8, ledge * 0.8), CFrame.new(x, ledgeY, z) * randomRotation(), Enum.Material.Slate, color)
		end
	end
	...
	for _, side in ipairs({ -1, 1 }) do
		...
		prop(arch, "ArchPillar", Vector3.new(26, 130, 26), CFrame.new(footprint.X, archCenter.Y - 20, footprint.Z) * CFrame.Angles(0, random() * math.pi, 0), Enum.Material.Slate, Color3.fromRGB(58, 66, 80))
	end
