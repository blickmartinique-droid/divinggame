...
- **Archipel des Profondeurs (`TitanShip` + `ArchipelDesProfondeurs`)** —
  ...
  - **`ArchipelWorld.server.lua`** — `Workspace/World/Underwater/
    ArchipelDesProfondeurs` (~2651 Parts depuis `ArchipelWorldData.lua`).
    ...
    .lua` : leurs propres sommets réels, groupés en chaîne de nœuds le long
    de l'axe dominant du maillage — pas un blockout inventé — reliés par
    des cylindres effilés plutôt que des boules isolées, pour un relief
    continu). Le rayon de chaque nœud ne compte que sa composante
    perpendiculaire à l'axe (au 85e percentile, pas le maximum brut), et
    tout le Terrain de l'archipel est effacé (retour à l'eau) au début de
    chaque run avant d'être reconstruit — Terrain est de la donnée voxel
    persistante qui s'accumule sinon d'un redémarrage à l'autre. Le reste
    (corail, éponges, cristaux, ruines, quai) devient des Parts, même
...
85 vérifications sur `TitanShip.server.lua`/`ArchipelWorld.server.lua` :
exécute réellement les deux scripts (pas juste leurs données) — c'est ce test
qui a débusqué deux vrais bugs avant qu'ils touchent le jeu : deux maillages
de la coque du Titan partageant la même boîte englobante (qui se seraient
reconstruits en un pavé solide bouchant une vraie entrée), et un rayon de
Terrain gonflé par une mesure qui confondait la longueur du segment avec son
épaisseur. Le test rejoue aussi le script DEUX FOIS de suite (comme un
redémarrage de partie) pour vérifier que le Terrain s'efface avant de se
reconstruire plutôt que de s'accumuler.
